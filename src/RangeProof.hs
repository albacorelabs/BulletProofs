...
generate_range_proof upperBound vals blinds h  = do
    ...
    let commitA = foldr1 (pointAdd crv) [pointMul crv alpha h, al `ecInner` gs, ar `ecInner` hs ]
        -- Commitment to blinding factors sl and sr
        commitS = foldr1 (pointAdd crv) [pointMul crv rho h, sl `ecInner` gs , sr `ecInner` hs]
        ...
        commitT1 = pointAdd crv (pointMul crv tau1 h) (pointBaseMul crv t1)
        commitT2 = pointAdd crv (pointMul crv tau2 h) (pointBaseMul crv t2)
...
verify_range_proof RangeProof{..} commits h = verifiedComm && ipVerify
    where
        ...
        pNoMu = foldr1 (pointAdd crv) [pointMul crv tx gx,pLHS,pointNegate crv $ pointMul crv mu h]
